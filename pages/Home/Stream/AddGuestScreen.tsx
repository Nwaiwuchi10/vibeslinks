...
export default function AddGuestScreen() {
  ...
  useEffect(() => {
    userService.getFollowers()
      ...
      .catch((err) => console.warn('Error loading followers for guest list:', err))
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={{ color: '#999' }}>No followers available to add.</Text>
        </View>
      ) : (
...
const styles = StyleSheet.create({
  ...
  backBtn: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#FFF', justifyContent: 'center', alignItems: 'center', ...Platform.select({ ios: { shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.08, shadowRadius: 4 }, android: { elevation: 2 } }) },
  headerTitle: { fontSize: 20, fontWeight: '800', color: '#111' },
  listContent: { paddingHorizontal: 24, paddingTop: 8, paddingBottom: 40 },
  userRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#EFEFEF' },
  userAvatar: { width: 50, height: 50, borderRadius: 25, marginRight: 14 },
  userName: { flex: 1, fontSize: 15, fontWeight: '600', color: '#111' },
  actionBtn: { paddingHorizontal: 22, paddingVertical: 10, borderRadius: 10 },
  addedBtn: { backgroundColor: '#E9174B' },
