...
const AdsBanner = ({ refreshKey, paddingHorizontal = 20 }: AdsBannerProps) => {
  ...
  useEffect(() => {
    homeService.getAdverts()
      ...
      .catch((err) => {
        console.warn('[AdsBanner] Error fetching adverts:', err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <TouchableOpacity
      style={[styles.adBannerContainer, { paddingHorizontal }]}
      activeOpacity={0.9}
      onPress={() => {
